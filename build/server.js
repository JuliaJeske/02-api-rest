...
async function transactionRoutes(app2) {
  ...
  app2.get(
    "/",
    {
      ...
      // executar essa função antes e valida a sessionId
    },
    async (req, reply) => {
  ...
  app2.post("/", async (req, reply) => {
    const createdTransactionBodySchema = import_zod2.z.object({
    ...
    if (!sessionId) {
      ...
      reply.cookie("sessionId", sessionId, {
        path: "/",
        // todas as rotas podem acessar
        maxAge: 1e3 * 60 * 60 * 24 * 7
        // tempo de expiração de cookies - 7dias
      });
    }
